...
ADD_DEPTH_WEIGHT=(0.9 0.5 0.3 0.7)
...
for WEIGHT in "${ADD_DEPTH_WEIGHT[@]}"; do
  echo "Running with additional depth weight: ${WEIGHT}"

  mkdir -p "${DATASET_DIR}/output"

  # Step 9: Train and Evaluate the Model with current WEIGHT
  run_and_log "python scripts/full_train.py --project_dir ${DATASET_DIR} --gt_point_cloud_constraints --additional_depth_maps --additional_depth_maps_weight ${WEIGHT} --extra_training_args '--exposure_lr_init 0.0 --eval' > ${RESULTS_FILE}" "Step 9: Train and Evaluate the Model w/o additional and constraint (weight ${WEIGHT})"

  # Step 10: Render Evaluation Results
  run_and_log "python render_hierarchy_weighted.py --segmentation_root_folder ${DATASET_DIR}/camera_calibration/rectified/semantic_segmentation/masks -s ${DATASET_DIR}/camera_calibration/aligned --model_path ${DATASET_DIR}/output --hierarchy ${DATASET_DIR}/output/merged.hier --out_dir ${DATASET_DIR}/output/renders --images ${DATASET_DIR}/camera_calibration/rectified/images --depths ${DATASET_DIR}/camera_calibration/rectified/depths --eval --scaffold_file ${DATASET_DIR}/output/scaffold/point_cloud/iteration_30000 >> ${RESULTS_FILE} 2>&1" "Step 10: Render Evaluation Results (weight ${WEIGHT})"

  # Rename the output folder
  NEW_OUTPUT_DIR="${DATASET_DIR}/output_ADD_DEPTH_WEIGHT_${WEIGHT//./_}" # Replaces '.' with '_' for folder name
