...
def _masked_ssim(img1, img2, mask, window, window_size, channel, size_average=True):
    # Ensure mask has same number of channels as images
    if mask.size(1) == 1 and channel > 1:
        mask = mask.expand(-1, channel, -1, -1)
    
    # Compute local statistics on full images (more stable near boundaries)
    mu1 = F.conv2d(img1, window, padding=window_size // 2, groups=channel)
    mu2 = F.conv2d(img2, window, padding=window_size // 2, groups=channel)
    ...
    # Compute variances and covariance on full images
    sigma1_sq = F.conv2d(img1 * img1, window, padding=window_size // 2, groups=channel) - mu1_sq
    sigma2_sq = F.conv2d(img2 * img2, window, padding=window_size // 2, groups=channel) - mu2_sq
    sigma12 = F.conv2d(img1 * img2, window, padding=window_size // 2, groups=channel) - mu1_mu2
    ...
    # Compute SSIM map on full images
    ssim_map = ((2 * mu1_mu2 + C1) * (2 * sigma12 + C2)) / ((mu1_sq + mu2_sq + C1) * (sigma1_sq + sigma2_sq + C2))
    
    # Apply mask to select only desired regions
